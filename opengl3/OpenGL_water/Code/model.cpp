...
void Model::unitize() {
    ...
    float minX = 100.0, maxX = -100.0, minY= 100.0, maxY = -100.0,
     ...
     for(int index = 0; index != vertices.size(); index++){
         if(vertices.at(index).x() > maxX)
             maxX = vertices.at(index).x();

         if(vertices.at(index).x() < minX)
             minX = vertices.at(index).x();

         if(vertices.at(index).y() > maxY)
             maxY = vertices.at(index).y();

         if(vertices.at(index).y() < minY)
             minY = vertices.at(index).y();

         if(vertices.at(index).z() > maxZ)
             maxZ = vertices.at(index).z();

         if(vertices.at(index).z() < minZ)
    ...
    for(auto index = vertices.begin(); index != vertices.end(); index++){
        index->operator +=(shift);
        index->operator /=(scale);
    }

    for(auto index = vertices_indexed.begin(); index != vertices_indexed.end(); index++){
        index->operator +=(shift);
        index->operator /=(scale);
    }
}
